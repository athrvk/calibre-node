...
const OPTION_KEY_PATTERN = /^[a-zA-Z0-9][a-zA-Z0-9-]*$/;
...
const buildCommand = (inputPath: string, outputPath: string, params: ConversionParams): Command => {
...
const handleConversion = async (params: ConversionParams, value: any) => {
    ...
    try {
        ...
        log([file, ...args].join(' '));
        const stdout = await execPromise(file, args);
